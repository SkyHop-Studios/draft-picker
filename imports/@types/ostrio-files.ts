declare module "meteor/ostrio:files" {
  ...
  type FileRefStatic<MetadataType> = {
    _id: string
    _collectionName: string
    _downloadRoute: string
    versions: {
  ...
  interface FilesCollectionConfig<MetadataType> {
    ...
    debug?: boolean;
    interceptDownload?: (http: { request: IncomingMessage, response: http.ServerResponse }, fileRef: FileRef<MetadataType>, version: string) => boolean;
  }
  ...
  class FileUpload {
    ...
    pipe(): void;
    start(): void;
    on<Event extends keyof FileUploadEvent>(event: Event, callback: FileUploadEvent[Event]): void;
  }

  type FileUploadEvent = {
    progress: (progress: number) => void
    end: (error: Meteor.Error | null, file: FileRef<any>) => void
  }
  ...
  class FilesCollection<MetadataType = { [x: string]: any }> {
    ...
    link(fileRef: FileRefStatic<MetadataType>, version?: string): string;
